...
/// it forwards a pointer rather than building a fresh one.
...
fn kernel_ret_forward_must_not_compact_older_frame_heap() {
    ...
    assert_ne!(
        ...
        U256::zero(),
        "kernel ret-forward must not be converted to a panic"
    );
...
/// The owned case must still compact — this is the rewrite of
/// `pop_frame_compacts_kept_heap_to_returndata_window`, which used a standalone page pushed
/// onto `heaps_i_am_keeping_alive` rather than the dying frame's own heap.
#[test]
fn pop_frame_compacts_dying_frames_own_heap_to_returndata_window() {
    let program: Program<(), TestWorld<()>> = Program::from_raw(vec![ret_instruction()], vec![]);
    let mut vm = VirtualMachine::new(
        kernel_address(),
        program.clone(),
        Address::zero(),
    ...
    // The dying frame's OWN heap — a dynamic page it exclusively owns.
    let own_heap = vm.state.current_frame.heap;
    let marker = U256::from(0xdead_beef_u64);
    vm.state.heaps.write_u256(own_heap, 0, marker);
    vm.state.heaps.write_u256(own_heap, 5000, marker);
    vm.state.heaps.write_u256(own_heap, 9000, marker);

    vm.pop_frame(Some(own_heap), Some((5000, 32)))
        .expect("nested frame must be present for pop");
    ...
    assert_eq!(
        vm.state.heaps[own_heap].read_u256(0),
        U256::zero(),
        "out-of-window bytes of the dying frame's OWN heap are freed"
    );
    assert_eq!(
        vm.state.heaps[own_heap].read_u256(9000),
        U256::zero(),
        "out-of-window bytes of the dying frame's OWN heap are freed"
    );
}

/// The same for the dying frame's own aux heap, since the ownership predicate names it.
#[test]
fn pop_frame_compacts_dying_frames_own_aux_heap_to_returndata_window() {
    let program: Program<(), TestWorld<()>> = Program::from_raw(vec![ret_instruction()], vec![]);
    let mut vm = VirtualMachine::new(
        kernel_address(),
        program.clone(),
    ...
    let own_aux_heap = vm.state.current_frame.aux_heap;
    let marker = U256::from(0xdead_beef_u64);
    vm.state.heaps.write_u256(own_aux_heap, 0, marker);
    vm.state.heaps.write_u256(own_aux_heap, 5000, marker);

    vm.pop_frame(Some(own_aux_heap), Some((5000, 32)))
        .expect("nested frame must be present for pop");

    assert_eq!(vm.state.heaps[own_aux_heap].read_u256(5000), marker);
    assert_eq!(vm.state.heaps[own_aux_heap].read_u256(0), U256::zero());
}
...
/// The precondition the deallocation defect below rests on: a kernel callee's ret-forward puts
/// a **live grandparent's** heap page into its caller's keep-alive list, which `pop_frame`
/// consumes as a list of pages that caller may free. The ownership gate keeps the page intact
/// at this point — only the later deallocation destroys it.
#[test]
fn kernel_ret_forward_puts_live_grandparent_heap_in_callers_keepalive() {
    ...
    // The ownership gate on compaction holds here: Hg is intact after B's pop.
    assert!(vm.state.heaps.contains(hg));
    assert_eq!(vm.state.heaps[hg].read_u256(0), marker);
    assert_eq!(vm.state.heaps[hg].read_u256(1024), marker);
    assert_eq!(vm.state.heaps[hg].read_u256(5000), marker);
}

/// When `A` then returns something other than `Hg`, `pop_frame`'s deallocation loop frees `Hg`
/// while `G` is still live, and `G` resumes reading zeros from its own heap.
///
/// This only *characterises* current behaviour: it asserts the CURRENT, DEFECTIVE outcome so a
/// fix to the keep-alive sink shows up as a failure here rather than passing unnoticed.
/// `zk_evm` never frees a memory page, so the correct outcome is an intact `Hg`. Reproduces
/// identically on this PR's base commit — it is a pre-existing defect the ownership fix does
/// not close; see
/// `security-review/heaps/05-keepalive-deallocate-live-dynamic-grandparent-heap.md`.
#[test]
fn keepalive_deallocation_frees_live_grandparent_heap() {
    ...
    // A returns something that is NOT Hg: a fresh MakeNewPointer into A's own heap, so the
    // `Some(heap) != heap_to_keep` test in `pop_frame`'s deallocation loop does not skip the
    // keep-alive entry.
    let mut a_return_abi = FatPointer {
        offset: 0,
    ...
    .into_u256();
    a_return_abi.0[3] = 0; // MakeNewPointer / ToHeap
    vm.state.registers[1] = a_return_abi;
...
/// The victim is two levels up. frame0 (initial) -> `G` (pushed frame, so its heap is a
/// dynamic page rather than the always-allocated bootloader heap) -> `A` (calldata heap `Hg`)
/// -> `B` (kernel), and `B` ret-forwards the pointer naming `Hg`.
///
/// Asserted immediately after `B`'s pop and before `A` returns: once `A` also returns, the
/// pre-existing keep-alive deallocation path frees `Hg` outright, for an unrelated reason (see
/// `keepalive_deallocation_of_live_grandparent_heap`).
#[test]
fn kernel_ret_forward_must_not_compact_grandparent_frame_heap() {
    let program: Program<(), TestWorld<()>> = Program::from_raw(vec![ret_instruction()], vec![]);
    let callee_program: Program<(), TestWorld<()>> =
        Program::from_raw(vec![ret_r1_instruction()], vec![]);
    ...
    // frame0 (initial, BOOTLOADER_HEAP_PAGE) -> G (dynamic heap Hg).
    let initial_heap = vm.state.current_frame.heap;
    vm.push_frame::<opcodes::Normal>(
        non_kernel_address(),
        program.clone(),
    ...
    let marker_in = U256::from(0x2222_2222_u64);
    let marker_hi = U256::from(0x3333_3333_u64);
    vm.state.heaps.write_u256(hg, 0, marker_lo);
    vm.state.heaps.write_u256(hg, 1024, marker_in);
    vm.state.heaps.write_u256(hg, 5000, marker_hi);

    // G -> A, with A's calldata pointer naming Hg.
    vm.push_frame::<opcodes::Normal>(
        non_kernel_address(),
        program,
        400_000,
    ...
    // A -> B (kernel), forwarding that same pointer down as B's calldata.
    vm.push_frame::<opcodes::Normal>(
        kernel_address(),
        callee_program,
        200_000,
    ...
    execute_one_instruction(&mut vm, &mut world, &mut ()); // B's `ret.ok r1`
    ...
    assert_eq!(vm.state.current_frame.heap, a_heap, "must be back in A");
    assert_eq!(vm.state.heaps[hg].read_u256(1024), marker_in);
    assert_eq!(
        vm.state.heaps[hg].read_u256(0),
        marker_lo,
        "a grandparent frame's heap must not be compacted"
    );
    assert_eq!(
        vm.state.heaps[hg].read_u256(5000),
        marker_hi,
        "a grandparent frame's heap must not be compacted"
    );
}
